...
- **Item ID**: (must be unique and use the existing `item_` prefix convention, e.g. `item_rusty_key`)
- **Name**: 
...
- **Takeable**: yes / no
- **Starting room**: (room ID where the item appears)
...
2. Add the item ID to the `items` array of the selected room in `src/data/rooms.ts`.
